...
data_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../dataset')
...
@torch.no_grad()
def evaluate(model, args, loader, set_len):
    cur_num = 0
...
def train(model, args, summary_writer):
    ...
    for epoch in range(args.num_epoch):
        ...
        valid_acc, valid_auc, valid_f1, valid_p, valid_r, valid_ap = \
            evaluate(model, args, valid_loader, valid_set_len)
        print()
        print(
        ...
        test_acc, test_auc, test_f1, test_p, test_r, test_ap = \
            evaluate(model, args, test_loader, test_set_len)
        print()
        print(
